...
const checkHourState = (day) => {
  ...
  if (
    hourPaymentState[day].start !== '' &&
    hourPaymentState[day].until !== ''
  ) {
...
const setupHourPaymentsControl = () => {
  ...
  domingoButton.addEventListener('click', () => {
    const hourPaymentState = JSON.parse(
      localStorage.getItem('hourPaymentState')
    );
    ...
    switch (hourPaymentState.active) {
      case 'domingo':
        ...
        localStorage.setItem(
          'hourPaymentState',
          JSON.stringify(hourPaymentState)
        );
